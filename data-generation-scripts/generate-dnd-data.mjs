...
async function generateDataInChunks(prompts, filename, combineResults = true) {
    ...
    try {
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: "You are a D&D 5e expert. Generate accurate JSON data based on official PHB rules. . Output only valid JSON array, no markdown, no code blocks, no explanations, no markdown formatting, just raw JSON."
          },
          {
            role: "user",
            content: prompts[i]
  ...
  console.log(`✓ Generated ${filename} with ${combineResults ? results.length : Object.keys(results).length} items`);
  return finalData;
}

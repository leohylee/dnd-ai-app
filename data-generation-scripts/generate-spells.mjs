      ...
      messages: [
        {
          role: 'system',
          content:
            'You are a D&D 5e expert. Provide accurate spell information in the exact format requested. Be concise.',
        },
        {
          role: 'user',
          content: prompt,
...
async function generateAllSpells() {
  ...
  for (const [levelKey, spellNames] of Object.entries(spellTemplate)) {
    const level =
      levelKey === 'cantrips' ? 0 : parseInt(levelKey.replace('level', ''))
    console.log(
      `Processing Level ${level} spells (${spellNames.length} spells)...`
    )
...
async function generateBasicSpellList() {
  ...
  console.log(
    `✓ Created spells-basic.json with ${basicSpells.length} spell entries`
  )
}
...
async function main() {
  ...
  if (args.includes('--basic')) {
    ...
    console.log(
      'To proceed with API calls, run with: node generate-spells-simple.mjs --yes'
    )
    console.log(
      'To generate basic list only, run with: node generate-spells-simple.mjs --basic'
    )
  }
}
